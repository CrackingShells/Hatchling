...
from hatchling.config.llm_settings import ELLMProvider, ModelInfo, ModelStatus
...
class ModelManagerAPI:
    ...
    async def check_provider_health(
        provider: ELLMProvider, settings: AppSettings = None
    ) -> bool:
        ...
        Returns:
            bool: Success flag and descriptive message.
        """
    ...
    async def list_available_models(
        provider: Optional[ELLMProvider] = None, settings: Optional[AppSettings] = None
    ) -> List[ModelInfo]:
    ...
    async def is_model_available(
        model_name: str, provider: ELLMProvider, settings: Optional[AppSettings] = None
    ) -> ModelInfo:
        ...
        try:
            ...
            for model in models:
                if model.name.lower() == model_name.lower():
                    logger.info(
                        f"Model '{model_name}' found for provider {provider.value}"
                    )
                    return model
    ...
    async def pull_model(
        model_name: str, provider: ELLMProvider, settings: AppSettings = None
    ) -> bool:
        ...
        if successful:
            ...
            if new_model_info not in settings.llm.models:
                logger.info(
                    f"Adding model {model_name} to available models for provider {provider.value}"
                )
                settings.llm.models.append(new_model_info)
            else:
                logger.info(
                    f"Model {model_name} is already in the available models for provider {provider.value}. No action taken."
                )
            ...
            for model_data in models_response.models:
                ...
                models.append(
                    ModelInfo(
                        ...
                        modified_at=str(model_data.modified_at),
        ...
        try:
            ...
            async for progress in await client.pull(model_name, stream=True):
                ...
                if status == "downloading":
                    ...
                    tqdm.write(
                        f"Downloading {model_name}: {percentage:.2f}% ({completed}/{total})"
                    )
